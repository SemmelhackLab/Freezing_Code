...
def heart_rate_pixelwise_trace(fish_dir, fish, date, trial_index, ROI, duration=1300):
    ...
    plt.figure(figsize=(10, 4))
    plt.plot(averaged_heart_rate)
    x1 = np.arange(900, 1000)
    plt.fill_between(x1, 4, -4, linewidth=1, color='lightcoral', alpha=0.2)
    plt.ylim(0, 4)
    print(np.std(averaged_heart_rate))
    plt.title('Averaged Heart Rate of T' + str(trial_index) + '_' + fish + '_' + date)
    plt.ylabel('Heart Rate per Second')
    plt.xlabel('Frames')
    plt.savefig(fish_dir + 'Averaged Heart Rate of_' + date + '_' + fish + '_T' + str(trial_index) + '.png')
    #     plt.show()
...
def get_pc_trial(fish_dir, threshold):
    ...
    for i in range(0, bradyinfo.shape[0]):
        ...
        fig.savefig(fish_dir_list[date][fish] + '\\eye_plotting_' + title + '.png', dpi=300)
